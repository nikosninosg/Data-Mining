...
'''
# 1h lysh, typwnei tis times tf-idf se mia sthlh
# settings that you use for count vectorizer will go here
tfidf_vectorizer = TfidfVectorizer(
    use_idf=True)  # Transforms text to feature vectors that can be used as input to estimator
cv = CountVectorizer()
# just send in all your docs here
tfidf_vectorizer_vectors = tfidf_vectorizer.fit_transform(f)  # Learn vocabulary and idf, return document-term matrix.
...
# 2h lysh, petaei error gia memory otan vazw to plhres arxeio
vectorizer = TfidfVectorizer()  # Transforms text to feature vectors that can be used as input to estimator
vectors = vectorizer.fit_transform(f)  # Learn vocabulary and idf, return document-term matrix.
